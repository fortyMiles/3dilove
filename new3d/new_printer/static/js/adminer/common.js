...
Commom.registerShow3dBtn = function(){
	$('.good-stl').click(function(){
		...
		Uplist.startStlProgressBar('show-3d-bar-container', 'show-3d-bar', 100, 10000, offTop -26, offLeft - 200, 150, 10);
...
Commom.clearBadImg = function(){
	$(".goods-container li").each(function(index){
		/*var regExp = new RegExp('\.(gif|jpg|jpeg|tiff|png)$','ig');*/
		/*var regExp = new RegExp('(gif|jpg|jpeg|bmp)','ig');*/

		for(var i=1; i<=3; i++){
			var thisImg = $(".goods-container li").eq(index).find('#good-img-'+i);
			var thisImgSrc = $(".goods-container li").eq(index).find('#good-img-'+i).attr('src');
			l('img:'+thisImgSrc);
			/*var result = regExp.test(thisImgSrc);*/
			/*l('re:'+result);*/

			var result = Commom.isImage(thisImgSrc);
			l(result);
...
Commom.isImage = function(imageName){
	var imageExts = ['jpg','jpeg', 'bmp', 'png'];
	var result = false

	for(var i = 0; i< imageExts.length; i++){

		if(imageName && imageName.indexOf(imageExts[i]) > 0){
			result = true;
			break;
		}
	}
